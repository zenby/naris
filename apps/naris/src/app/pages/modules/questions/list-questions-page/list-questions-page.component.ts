...
export class ListQuestionsPageComponent {
  ...
  constructor(
    ...
    this.question$ = this.store$.of(this.route.snapshot.data['questions']).pipe(
  ...
  questionDelete(workbook: QuestionModel): void {
    this.bus$.publish(new CommandDelete(this.questionId, workbook, { qid: workbook.id }));
  }
  ...
  createQuestion(): void {
    this.bus$.publish(new CommandNew(this.questionId));
  }
